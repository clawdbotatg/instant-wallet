...
TOKEN = "0x5FbDB2315678afecb367f032d93F642f64180aa3"      # MockUSDC on the local anvil
DECIMALS = 6
...
def state():
    ...
    bal = "%.2f" % balance
    signers = [
        {"signerId": "0x4d5c3a8f2e1b0c9d8e7f6a5b4c3d2e1f0a9b8c7d", "kind": 0, "role": 0, "dailyLimit": str(500 * 10 ** DECIMALS), "label": "iPhone Face ID"},
    ]
    sid = _signer_id()
    if paired and sid:
        signers.insert(0, {"signerId": sid, "kind": 1, "role": 1, "dailyLimit": "0", "label": device.get("name", "device")})
    return {
        "chain": {"id": CHAIN_ID, "name": CHAIN_NAME},
        "wallet": {"address": WALLET, "ensName": ENS, "balance": str(int(balance * 10 ** DECIMALS)), "balanceFormatted": bal},
        "token": {"address": TOKEN, "symbol": "USDC", "decimals": DECIMALS},
...
def inject(kind="transfer", tamper=False, **over):
    global nonce
    ...
    if kind == "transfer":
        r.update({"token": TOKEN, "tokenSymbol": "USDC", "tokenDecimals": DECIMALS, "to": ATG, "toName": "vault.atg.eth",
                  "amount": str(2000 * 10 ** DECIMALS), "amountFormatted": "2000.00", "fee": str(20000)})
        r.update(over)
        digest = eip712.transfer_digest(CHAIN_ID, WALLET, r["token"], r["to"], int(r["amount"]), int(r["fee"]), nonce, deadline)
    elif kind == "execute":
        calls = [
        ...
        r.update({"qx": PHONE_QX, "qy": PHONE_QY, "signerKind": 0, "role": 0, "dailyLimit": str(500 * 10 ** DECIMALS),
                  "credentialIdHash": "0x" + "ab" * 32, "label": "iPhone Face ID"})
        r.update(over)
        digest = eip712.add_signer_digest(CHAIN_ID, WALLET, r["qx"], r["qy"], r["signerKind"], r["role"], int(r["dailyLimit"]), r["credentialIdHash"], nonce, deadline)
    elif kind == "updateSigner":
        r.update({"targetSignerId": "0x4d5c3a8f2e1b0c9d8e7f6a5b4c3d2e1f0a9b8c7d", "role": 0, "dailyLimit": str(750 * 10 ** DECIMALS)})
        r.update(over)
        digest = eip712.update_signer_digest(CHAIN_ID, WALLET, r["targetSignerId"], r["role"], int(r["dailyLimit"]), nonce, deadline)
    elif kind == "removeSigner":
        r.update({"targetSignerId": "0x4d5c3a8f2e1b0c9d8e7f6a5b4c3d2e1f0a9b8c7d"})
        r.update(over)
