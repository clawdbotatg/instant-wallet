...
//   node emu/server.mjs [--port 4242] [--app http://localhost:3000] [--open]
...
const APP = opt("app", process.env.EMU_APP || "http://localhost:3000");
...
const STATIC = [
  ...
  ["/stl/", join(ROOT, "hardware/case/zez0000")],
];
...
server.listen(PORT, () => {
  console.log(`instant-wallet device emulator  http://localhost:${PORT}/   app proxy -> ${APP}`);
  if (OPEN) openBrowser();
});
